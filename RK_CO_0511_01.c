...
int main (void)
{
  ...
  for(int ii = 1; ii <= NN; ii++) printf("xx[%3d]=  %10.3lf\n",ii,xx[ii]);
 ...
 /* initial condition*/
  ...
  //judgement of the end of slipping
    ...
    for(int ii = 0; ii < no_of_earthquake; ii++)  printf("No. = %5d tstt= %10.3lf   tend=   %10.3lf  mag = %10.3lf\n", ii, eqtimestt[ii], eqtimesend[ii],eqmagnitude[ii]);
 printf("No. = %5d \n", no_of_earthquake);
...
void derivyy(double time, double *xx, double *yy, double *ddyy)
{
  ...
  /*for (int ii = 1; ii <= NN; ii++)    ddyy[ii] = xx[ii + 1] + xx[ii - 1] - 2.0 * xx[ii]+KKZ*(time-xx[ii]);*/
